...
// Returns a promise that resolves to an ArrayBuffer : asynchronous
function sha256(plain) {
  const encoder = new TextEncoder(); // create instance of TextEncoder
  const data = encoder.encode(plain);  // encode to binary data : convert string to Uint8Array
  return window.crypto.subtle.digest("SHA-256", data); // compute SHA-256 hash of this binary data
}
  ...
  $.ajax({
    ...
    complete: function (data) {
      ...
      document.getElementById("myButton").onclick = function () {
        ...
        $.ajax({
          url: base_url+"api/authenticateadmin/",
          type: "POST",
